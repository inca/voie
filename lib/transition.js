...
export default class Transition {
  ...
  run() {
    return this.goUpstream().then(() => this.goDownstream());
  }
  ...
  goUpstream() {
    ...
    if (this.dstState.includes(state)) {
      ...
      let paramsMatch = Object.keys(ctx.params).every(key => ctx.params[key] == this.params[key]);
      if (paramsMatch) {
        return Promise.resolve();
      }
    }
    return Promise.resolve().then(() => state.leave(ctx)).then(() => {
  ...
  goDownstream() {
    ...
    return Promise.resolve().then(() => nextState.enter(nextContext)).catch(err => nextState.handleError(err, nextContext)).then(obj => {
      obj = obj || {};
